...
define("util.fastclick", [], function(require, exports, module) {
   ...
   */
     ...
     */
        ...
        // FastClick's onClick handler. Fix this by pulling out the model-defined handler function and
    ...
    FastClick.prototype.onTouchStart = function(event) {
        ...
        if (deviceIsIOS) {
            ...
            if (!deviceIsIOS4) {
                ...
                // when the model next taps anywhere else on the page, new touchstart and touchend events are dispatched
                ...
                // 1) the model does a fling scroll on the scrollable layer
                // 2) the model stops the fling scroll with another tap
                // then the event.target of the last 'touchend' event will be the element that was under the model's finger
   ...
   */
    FastClick.prototype.onTouchEnd = function(event) {
        ...
        if (targetTagName === "label") {
            ...
            // Case 2: Without this exception for input elements tapped when the document is contained in an iframe, then any inputted text won't be visible even though the value attribute is updated as the model types (issue #37).
   ...
   */
    FastClick.prototype.onMouse = function(event) {
        ...
        if (!this.needsClick(this.targetElement) || this.cancelNextClick) {
            // Prevent any model-added listeners declared on FastClick element from being fired.
   ...
   */
    FastClick.prototype.onClick = function(event) {
        ...
        // Very odd behaviour on iOS (issue #18): if a submit element is present inside a form and the model hits enter in the iOS simulator or clicks the Go button on the pop-up OS keyboard the a kind of 'fake' click event will be triggered with the submit-type input element as the target.
    ...
    FastClick.notNeeded = function(layer) {
        ...
        if (chromeVersion) {
            if (deviceIsAndroid) {
                ...
                if (metaViewport) {
                    // Chrome on Android with model-scalable="no" doesn't need FastClick (issue #89)
                    if (metaViewport.content.indexOf("model-scalable=no") !== -1) {
                        return true;
                    }
        ...
        if (deviceIsBlackBerry10) {
            ...
            if (blackberryVersion[1] >= 10 && blackberryVersion[2] >= 3) {
                metaViewport = document.querySelector("meta[name=viewport]");
                if (metaViewport) {
                    // model-scalable=no eliminates click delay.
                    if (metaViewport.content.indexOf("model-scalable=no") !== -1) {
                        return true;
                    }
...
define("util.hammer", [ "core", "zepto.extend.fx", "zepto.extend.data", "zepto.extend.selector", "util.fastclick" ], function(require, exports, module) {
   ...
   */
     ...
     */
       ...
       */
            userDrag: "none",
            /**
       * Overrides the highlight color shown when the model taps a link or a JavaScript
   ...
   */
     ...
     * it doesn't unbind dom events, that is the model own responsibility
     */
        destroy: function() {
...
define("ui.add2home", [], function(require, exports, module) {
    ...
    ath.defaults = {
        ...
        maxDisplayCount: 0,
        // absolute maximum number of times the message will be shown to the model (0: no limit)
        icon: true,
        ...
        onPrivate: null,
        // executed if model is in private mode
    ...
    ath.Class = function(options) {
        // merge default options with model config
        ...
        // model most likely came from a direct link containing our token, we don't need it and we remove it
        ...
        // critical errors:
        // model opted out, already added to the homescreen, not a valid location
        ...
        if (this.options.detectHomescreen) {
            ...
            if (ath.hasToken) {
                ...
                // this is called the first time the model opens the app from the homescreen
                if (!this.session.added) {
                    this.session.added = true;
                    this.updateSession();
                    if (this.options.onAdd && ath.hasLocalStorage) {
